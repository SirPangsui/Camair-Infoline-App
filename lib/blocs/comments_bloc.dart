...
import 'package:cmer_infoline/utils/toast.dart';
...
class CommentsBloc extends ChangeNotifier{
  ...
  Future _getDate() async {
    ...
    String _date = DateFormat('dd MMMM yy, HH:mm').format(now);
  ...
  Future reportComment (String? postDocumentName, String? uid, String? comment, String? commnetTimestamp) async{
    final SharedPreferences sp = await SharedPreferences.getInstance();
    ...
    await _getDate().then((value)async{
      ...
      final DocumentReference ref = firestore.collection('user_reports').doc(_documentName).collection('reported_list').doc(newTimestamp);
